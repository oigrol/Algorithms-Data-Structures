...
/*In questa variante, stiamo ordinando i puntatori agli elementi del vettore, piuttosto che gli elementi stessi del vettore originale.
 *Questo ci permette di mantenere intatto l'ordine originale dei dati. Infatti, ristampando il vettore dopo aver applicato i vari ordinamenti sui puntatori,
 *notiamo che il vettore ci viene trasmesso nella sua forma originale, a differenza del caso in cui, ordinando direttamente gli elementi del vettore, si andava a
 *modificare l'ordine degli elementi del vettore in modo permanente*/
...
void selezionaDati(corse_t *corse[], corse_t *B[], corse_t *perData[], corse_t *perCodice[], corse_t *perPartenza[], corse_t *perArrivo[], int N, comando_e comando);
...
int main(void) {
    ...
    corse_t *corse_original[MAXCORSE];
    corse_t *perData[MAXCORSE];
    corse_t *perCodice[MAXCORSE];
    corse_t *perPartenza[MAXCORSE];
    corse_t *perArrivo[MAXCORSE];

    for (int i = 0; i < num_tratte; i++) {
        //l'i-esimo elemento del vettore di puntatori deve contenere l'indirizzo dell'i-esimo elemento del vettore corse originale
        perData[i] = &corse[i];
        perCodice[i] = &corse[i];
        perPartenza[i] = &corse[i];
        perArrivo[i] = &corse[i];
        corse_original[i] = &corse[i];
        //andrò poi a fare in modo che gli indirizzi si riferiscano al vettore originale però secondo il corretto ordine, seppur il vettore originale corse rimarrà di ordine invariato
    }

    while (comando != r_fine) {
        comando = leggiComando();
        selezionaDati(corse_original, B, perData, perCodice, perPartenza, perArrivo, num_tratte, comando);
    }
