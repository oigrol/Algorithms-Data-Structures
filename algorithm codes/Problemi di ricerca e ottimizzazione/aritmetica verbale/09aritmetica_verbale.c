...
int w2n(alpha *lettere, char *str) { // converti stringa di caratteri in intero
  ...
  if (lettere[trova_indice(lettere, str[0])].val == 0) //al carattere più significativo str[0] facciamo corrispondere 0? Non lo vogliamo
    return -1;

  for(i=0; i < l; i++)
    valore = valore * 10 + lettere[trova_indice(lettere, str[i])].val; //numerazione posizionale
  return valore;
}
...
int contr_sol(alpha *lettere, char *str1, char *str2, char *str3) { //escludendo i casi con cifra piu' significativa nulla, controlla se n1+n2=n3
  ...
  if (n1 == -1 || n2 == -1 || n3 == -1)
    return 0;
  return ((n1 + n2) == n3); //verifica  -> successo se somma esatta
}
...
int disp(alpha *lettere, int *mark, int pos, char *str1, char *str2, char *str3) {
  ...
  for(i=0;i < base; i++) { //disposizioni semplici -> opero sulle scelte legate alla base
    if (mark[i]==0) { //se cifra i(da 0 a 9) non ancora presa
        lettere[pos].val = i; //la prendo -> valore lo metto in val a pos
        mark[i] = 1; //presa
...
void setup(alpha *lettere, char *str1, char *str2, char *str3) {
  ...
  for(i=0; i<l1; i++) {
    if (trova_indice(lettere, str1[i]) == -1) //non c'è ancora il carattere nella tabella di simboli -> lo piazzo dentro
...
int main(void) {
  ...
  char str1[LUN_MAX], str2[LUN_MAX], str3[LUN_MAX+1]; //+1 perchè avremo al massimo un riporto nella somma
  int mark[base] = {0};
  int i;
