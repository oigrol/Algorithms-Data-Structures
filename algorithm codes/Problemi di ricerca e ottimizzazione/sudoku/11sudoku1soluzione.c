...
********************************************************************/
int disp_ripet(int pos) {  // a differenza di prima la funzione ritorna un intero perchè appena ritorna 1 mi fermerò
  int i, j, t;
  ...
  for (t=1; t<=n; t++) {
    schema[i][j] = t;
    if (controlla(pos, t))
      if (disp_ripet(pos+1)==1) //diverso da prima ritorna qualcosa
        return 1;
    schema[i][j] = 0;
  }
  return 0;
